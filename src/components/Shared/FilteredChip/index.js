...
const FilteredChip = ({ name }) => {
    ...
    return (
        <div className="flex justify-center items-center gap-2 bg-chipColor w-auto p-2 rounded-full mb-3 lg:mb-0">
            ...
            <button className="flex justify-center items-center h-6 w-6 sm:h-8 sm:w-8 bg-secondaryColor rounded-full" ><img src={icons.cross} al className="w-3 h-3" onClick={() => removeFilter(name)} /></button>
        </div>
    )
}
