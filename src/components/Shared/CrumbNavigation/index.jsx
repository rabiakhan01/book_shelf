    ...
    return (
        <div className="flex gap-2 items-center">
            ...
            <div className="flex gap-2">
                <div className="flex gap-2 items-center">
                    <button onClick={() => handelNavigate('/cart')} className="cursor-pointer hover:text-textYellowColor uppercase">Back</button>
                    ...
                    navigationArray.map((bread, index) => {
                        return (
                            <div className="flex gap-2 items-center" key={index}>
                                <button onClick={() => handelNavigate(bread.path, bread.id)} className={`cursor-pointer ${state.isActive == true && state.id == bread.id ? 'text-textYellowColor' : 'text-textSecondaryColor'} uppercase`}>{bread.name}</button>
